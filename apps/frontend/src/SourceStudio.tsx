...
import { api } from "./api";
import type { DataSource, Job } from "./types";
...
export function SourceStudio({ onSynced }: { onSynced: () => Promise<void> }) {
  ...
  async function sync(source: DataSource) {
    try {
      await api.syncSource(source.id, `manual-${Date.now()}`, source.dataset_id ? "incremental" : "full");
