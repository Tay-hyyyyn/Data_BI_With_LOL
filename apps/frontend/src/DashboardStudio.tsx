...
import type { Dashboard, Dataset, Preview, Profile } from "./types";

type Widget = { id: string; title: string; type: "kpi" | "bar" | "line" | "histogram" | "scatter" | "boxplot" | "heatmap"; column: string; dimension?: string; secondary?: string };
...
export function DashboardStudio({ dataset, profile, preview }: { dataset: Dataset | null; profile: Profile | null; preview: Preview | null }) {
  ...
  useEffect(() => { setColumn(numeric[0]?.name ?? ""); setSecondary(numeric[1]?.name ?? numeric[0]?.name ?? ""); setDimension(categorical[0]?.name ?? ""); setFilterColumn(categorical[0]?.name ?? ""); }, [profile?.version_id]);
  ...
  function add() {
    ...
    setWidgets((current) => [...current, { id: crypto.randomUUID(), title: primary, type: kind, column: primary, dimension: needsGroup ? dimension || undefined : undefined, secondary: ["scatter", "heatmap"].includes(kind) ? secondary || undefined : undefined }]);
  }
  ...
  async function save() {
    setMessage("");
    const payload = { name, widgets: widgets.map((widget) => ({ ...widget, dataset_id: dataset?.id })), filters: filterColumn ? [{ column: filterColumn, value: filterValue }] : [] };
  ...
  return <section className="studio">
    <div className="studio-toolbar"><div><p>대시보드 편집기</p><input aria-label="대시보드 이름" value={name} onChange={(event) => setName(event.target.value)}/><select className="saved-dashboard" aria-label="저장된 대시보드" value={dashboardId} onChange={(event) => void openSaved(event.target.value)}><option value="">새 대시보드</option>{saved.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></div><div className="widget-controls"><select aria-label="위젯 유형" value={kind} onChange={(event) => { const next = event.target.value as Widget["type"]; setKind(next); setSecondary(next === "heatmap" ? categorical[1]?.name ?? categorical[0]?.name ?? "" : numeric[1]?.name ?? numeric[0]?.name ?? ""); }}><option value="kpi">KPI</option><option value="bar">막대 차트</option><option value="line">선 차트</option><option value="histogram">히스토그램</option><option value="scatter">산점도</option><option value="boxplot">박스플롯</option><option value="heatmap">히트맵</option></select>{kind !== "heatmap" && <select aria-label="지표 컬럼" value={column} onChange={(event) => setColumn(event.target.value)}>{numeric.map((item) => <option key={item.name}>{item.name}</option>)}</select>}{["bar", "line", "boxplot", "heatmap"].includes(kind) && <select aria-label={kind === "heatmap" ? "첫 번째 범주 컬럼" : "그룹 컬럼"} value={dimension} onChange={(event) => setDimension(event.target.value)}><option value="">그룹 없음</option>{categorical.map((item) => <option key={item.name}>{item.name}</option>)}</select>}{["scatter", "heatmap"].includes(kind) && <select aria-label="두 번째 컬럼" value={secondary} onChange={(event) => setSecondary(event.target.value)}>{(kind === "heatmap" ? categorical : numeric).map((item) => <option key={item.name}>{item.name}</option>)}</select>}<button onClick={add}><Plus size={16}/> 위젯 추가</button><button className="primary" onClick={() => void save()}><Save size={16}/> 저장</button>{dashboardId && <><button onClick={() => void clone()}>복제</button><button onClick={() => void publish()}>게시</button></>}</div></div>
...
function WidgetCard({ datasetId, widget, filter, onRemove, ...drag }: { datasetId: string; widget: Widget; filter: { column: string; value: string } | null; onRemove: () => void; draggable: boolean; onDragStart: () => void; onDragOver: (event: React.DragEvent) => void; onDrop: () => void }) {
  const [rows, setRows] = useState<Array<{ category?: unknown; value: number | null }>>([]);
  ...
  useEffect(() => {
    ...
    const request = advanced
      ...
      : api.queryDataset(datasetId, { measure: widget.column, aggregation: "sum", dimension: widget.dimension || null, filters });
    request
    ...
    return () => { active = false; };
  }, [datasetId, widget.column, widget.dimension, widget.secondary, widget.type, filter?.column, filter?.value]);
  const total = rows[0]?.value ?? 0;
  const option = useMemo(() => ({ grid: { left: 45, right: 12, top: 12, bottom: 28 }, xAxis: { type: "category" as const, data: rows.map((row, index) => String(row.category ?? index + 1)) }, yAxis: { type: "value" as const }, series: [{ type: widget.type === "line" ? "line" as const : "bar" as const, showSymbol: false, data: rows.map((row) => row.value), itemStyle: { color: "#3977e8" }, lineStyle: { color: "#36a889" } }] }), [widget.type, rows]);
  return <article className="dashboard-widget" {...drag}><div className="widget-head"><GripVertical size={17}/><b>{widget.title}</b><button aria-label={`${widget.title} 삭제`} onClick={onRemove}><Trash2 size={15}/></button></div>{error ? <div className="notice">{error}</div> : widget.type === "kpi" ? <div className="widget-kpi"><span>전체 합계</span><strong>{new Intl.NumberFormat("ko-KR", { maximumFractionDigits: 2 }).format(total)}</strong><small>게시 버전 전체 행 기준</small></div> : <EChart option={chartSpec ?? option}/>}</article>;
}
