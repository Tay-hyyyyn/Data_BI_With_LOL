...
export function DashboardStudio({ dataset, profile, preview }: { dataset: Dataset | null; profile: Profile | null; preview: Preview | null }) {
  ...
  useEffect(() => {
    let active = true;
    if (!dataset || !filterColumn) { setFilterValues([]); return; }
    api.distinctValues(dataset.id, filterColumn).then((result) => { if (active) setFilterValues(result.values); }).catch(() => { if (active) setFilterValues([]); });
    return () => { active = false; };
  }, [dataset?.id, filterColumn]);
  ...
  return <section className="studio">
    ...
    <div className="filter-bar"><b>공통 필터</b><select value={filterColumn} onChange={(event) => { setFilterColumn(event.target.value); setFilterValue(""); }}><option value="">사용 안 함</option>{categorical.map((item) => <option key={item.name}>{item.name}</option>)}</select><select value={filterValue} disabled={!filterColumn} onChange={(event) => setFilterValue(event.target.value)}><option value="">전체</option>{filterValues.map((item) => <option key={item.value} value={item.value}>{item.value} ({item.count.toLocaleString()})</option>)}</select><span>{filterColumn ? "게시 데이터 전체 기준" : `${filteredRows.length.toLocaleString()}행 적용`}</span></div>
