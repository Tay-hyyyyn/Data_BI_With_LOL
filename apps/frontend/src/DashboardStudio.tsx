...
export function DashboardStudio({ dataset, profile, preview }: { dataset: Dataset | null; profile: Profile | null; preview: Preview | null }) {
  ...
  function add() {
    ...
    setWidgets((current) => [...current, { id: crypto.randomUUID(), title: primary, type: kind, column: primary, aggregation, dimension: needsGroup ? dimension || undefined : undefined, secondary: ["scatter", "heatmap"].includes(kind) ? secondary || undefined : undefined }]);
  }
...
function WidgetCard({ datasetId, widget, filter, onRemove, ...drag }: { datasetId: string; widget: Widget; filter: { column: string; value: string } | null; onRemove: () => void; draggable: boolean; onDragStart: () => void; onDragOver: (event: React.DragEvent) => void; onDrop: () => void }) {
  ...
  useEffect(() => {
    ...
    const request = advanced
      ...
      : api.queryDataset(datasetId, { measure: widget.column, aggregation: widget.aggregation || "sum", dimension: widget.dimension || null, filters });
    request
