...
import { Activity, ArrowUpRight, BarChart3, Database, FileUp, Link2, LoaderCircle, Search, Sparkles, Table2 } from "lucide-react";
import { api } from "./api";
...
export default function App() {
  const [view, setView] = useState<"data" | "prep" | "dashboard" | "pipeline">("data");
  ...
  const viewTitle = { data: active?.name ?? "새 데이터로 분석을 시작하세요", prep: "전처리 레시피", dashboard: "대시보드 편집기", pipeline: "파이프라인 작업" }[view];
  ...
  return <div className="shell">
    <aside className="sidebar">
      ...
      <nav><button className={view === "data" ? "nav-active" : ""} onClick={() => setView("data")}><Database size={17}/> 데이터 허브</button><button className={view === "prep" ? "nav-active" : ""} onClick={() => setView("prep")}><Table2 size={17}/> 전처리 레시피</button><button className={view === "dashboard" ? "nav-active" : ""} onClick={() => setView("dashboard")}><BarChart3 size={17}/> 대시보드</button><button className={view === "pipeline" ? "nav-active" : ""} onClick={() => setView("pipeline")}><Activity size={17}/> 파이프라인</button></nav>
    ...
    <main>
      ...
      {view === "prep" ? <PrepStudio dataset={active} profile={profile} onPublished={async () => { await refresh(); if (active) { const [p, rows] = await Promise.all([api.profile(active.id), api.preview(active.id)]); setProfile(p); setPreview(rows); } }}/> : view === "dashboard" ? <DashboardStudio dataset={active} profile={profile} preview={preview}/> : view === "pipeline" ? <PipelineStudio dataset={active} profile={profile}/> : !active ? <section className="empty"><div><FileUp size={30}/></div><h2>첫 데이터셋을 올려보세요</h2><p>CSV, Excel, Parquet 파일을 원자적 버전으로 저장하고 바로 탐색할 수 있습니다.</p><button className="primary" onClick={() => fileRef.current?.click()}>파일 선택</button></section> : <>
