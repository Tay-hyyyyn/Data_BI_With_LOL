...
export function PipelineStudio({ dataset, profile }: { dataset: Dataset | null; profile: Profile | null }) {
  ...
  const load = async () => { const [nextJobs, nextPipelines] = await Promise.all([api.listJobs(), api.listPipelines()]); setJobs(nextJobs); setPipelines(nextPipelines); };
  useEffect(() => { void load(); }, []);
