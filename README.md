...
- 순서형 전처리 API(선택·이름변경·형변환·결측·중복·필터·계산·집계·피벗·안전한 데이터셋 조인)
...
기존 비공개 경기 표본이 있으면 그 경기의 참가자를 seed로 사용해 중복 없이 최대 30건을 추가 수집할 수 있습니다. Development Key의 장기 요청 한도를 넘지 않도록 보수적으로 실행하며, 키가 만료되었으면 기존 데이터는 변경하지 않고 재발급 안내와 함께 중단합니다.

```bash
python scripts/expand_lol_sample.py --max-new-matches 30
```

Riot API Key 없이도 Data Dragon 기반 아이템·챔피언·룬과 정적 골드 가치 모델은 수집할 수 있습니다. `LoL static stat value trend` 데이터셋은 패치와 스탯을 차원으로 하여 기준가격·Ridge·NNLS·bootstrap 구간을 일반 BI의 선 차트로 비교하는 용도입니다. 공격속도와 치명타는 Data Dragon의 비율값이 아니라 사람이 읽는 퍼센트포인트 단위로 저장합니다. 즉 `10`은 10%이며, 기준가격은 각각 25 G/%와 40 G/%입니다.
...
LoL 실험실의 **통합 사례 대시보드**는 정적 아이템 가치, 패치별 골드·AP 추세, 표본 경기 수, 가장 최신 패치의 골드 구간별 관찰 승률을 하나의 비공개 대시보드로 생성합니다. 관찰 승률은 현재 수집된 표본의 기술통계이며 인과효과 또는 일반 메타 결론으로 해석하면 안 됩니다.
