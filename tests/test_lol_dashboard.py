...
from app.schemas import LolStarterDashboardRequest
from app.services import lol as lol_service
...
def test_lol_starter_dashboard_uses_patch_specific_marts(monkeypatch) -> None:
    ...
    monkeypatch.setattr(lol_service, "get_dataset_by_name", lambda name, _source: datasets.get(name))
    captured = {}
    ...
    monkeypatch.setattr(lol_service, "save_or_update_dashboard", fake_save)

    result = lol_service.create_starter_dashboard(LolStarterDashboardRequest(patch="16.18.1"))
...
def test_lol_patch_trend_dashboard_uses_trend_and_coverage_marts(monkeypatch) -> None:
    ...
    monkeypatch.setattr(lol_service, "get_dataset_by_name", lambda name, _source: datasets.get(name))
    captured = {}
    monkeypatch.setattr(lol_service, "save_or_update_dashboard", lambda name, widgets, filters=None: captured.update(name=name, widgets=widgets) or SimpleNamespace(name=name, widgets=widgets))

    result = lol_service.create_patch_trend_dashboard()
